...
#include <time.h>
#include <windows.h>
//#include <unistd.h> // Se estiver no Linux / Mac -> função sleep(x)..
...
void criarConta(){
    ...
    //Dia
    if(tm.tm_mday < 10){
        sprintf(dia, "0%d", tm.tm_mday); // 1, 2, 3, 4, 5, 6, 7, 8, 9 -> 16/02/2021
    }else{
        sprintf(dia, "%d", tm.tm_mday);
    }
    //Mês
    if((tm.tm_mon + 1) < 10){ // 16/02/2021
    ...
    strcat(data_cadastro, dia); //16
    strcat(data_cadastro, "/"); // 16/
    strcat(data_cadastro, mes); //16/02
    strcat(data_cadastro, "/"); //16/02/
    strcat(data_cadastro, ano); //16/02/2021
...
void efetuarSaque(){
    if(contador_contas > 0){
        ...
        if(conta.numero == numero){
            ...
            printf("Nao foi encontrado uma conta com numero %d\n", numero);
        }
...
void listarContas(){
    if(contador_contas > 0){
        ...
        printf("Nao existem contas cadastradas no momento. \n");
    }
    Sleep(4);
    menu();
}
